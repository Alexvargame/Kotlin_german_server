...
data class UiButtonStateInBeruf(
    val wordId: Long,
    val text: String,
    var color: Color
)
enum class ColumnTypeInBeruf {
    LEFT, RIGHT
}

class ExercisesBerufWordsPairViewModel(
    private val repo: ExerciseBerufWordsPairRepository
) : ViewModel() {

    var exercises by mutableStateOf<List<WordsPairExercise>>(emptyList())
        private set
    var leftButtons by mutableStateOf<List<UiButtonStateInBeruf>>(emptyList())
        private set
    var rightButtons by mutableStateOf<List<UiButtonStateInBeruf>>(emptyList())

    var selectedLeft: UiButtonStateInBeruf? = null
    var selectedRight: UiButtonStateInBeruf? = null
    ...
    fun loadExercises() {
        viewModelScope.launch {
            ...
            leftButtons = words.map { UiButtonStateInBeruf(it.id, it.german, Color.LightGray) }
            rightButtons =
    ...
    fun onButtonClick(wordId: Long, column: ColumnTypeInBeruf) {
        when (column) {
            ColumnTypeInBeruf.LEFT -> {
