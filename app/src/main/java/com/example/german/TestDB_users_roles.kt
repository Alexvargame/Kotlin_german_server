...
class TestDb_users_roles(private val context: Context) {
    ...
    fun testusersroles() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val testUserRole = UserRole(name = "Тестовая role", description = "Описание roel")
            userRoleDao.insert(testUserRole)

            val newUserRole = UserRole(name = "Новая тестовая roel", description = "Описание новой role")
            userRoleDao.insert(newUserRole)
            Log.d("TEST_DB", "Новая тестовая role вставлена")
            ...
            val userRoleToUpdate = userRoles[0].copy(name = "Обновлённая тестовая лекция")
