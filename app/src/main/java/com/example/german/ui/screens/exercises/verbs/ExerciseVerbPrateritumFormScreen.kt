...
@Composable
fun ExerciseVerbPrateritumFormScreen(
    navController: NavController,
    userProfileViewModel: UserViewModel,
    viewModel: ExercisesVerbPrateritumViewModel,

    ) {
    LaunchedEffect(Unit) {
        viewModel.loadExercises()
        viewModel.exercises.forEach { ex ->
            Log.d("Verb_present", "WordId: ${ex.infinitive}")
        }
    }
...
// Можно для отладки
                ...
                navController.navigate(
                    "exercise_verb_prateritum_form_result_screen/${result.correctCount}/${result.totalQuestions}"
                )
